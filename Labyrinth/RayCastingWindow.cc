...
RayCastingWindow::RayCastingWindow(int width, int height) // @suppress("Class members should be properly initialized")
: Window(width, height)
{
...
void RayCastingWindow::render()
{
	...
	SDL_SetRenderDrawColor(_renderer.get(), 124, 238, 255, 255);
	SDL_RenderFillRect(_renderer.get(), &r_sky);
	SDL_SetRenderDrawColor(_renderer.get(), 124, 90, 123, 205);
	...
	SDL_SetRenderDrawColor(_renderer.get(), 128, 0, 255 , 255);
